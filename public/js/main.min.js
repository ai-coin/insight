...
var defaultLanguage = localStorage.getItem("insight-language") || "en",
    defaultCurrency = localStorage.getItem("insight-currency") || "XAI";
angular.module("insight", ["ngAnimate", "ngResource", "ngRoute", "ngProgress", "ui.bootstrap", "ui.route", "monospaced.qrcode", "gettext", "angularMoment", "insight.system", "insight.socket", "insight.blocks", "insight.transactions", "insight.address", "insight.search", "insight.status", "insight.connection", "insight.currency", "insight.messages"]), angular.module("insight.system", []), angular.module("insight.socket", []), angular.module("insight.blocks", []), angular.module("insight.transactions", []), angular.module("insight.address", []), angular.module("insight.search", []), angular.module("insight.status", []), angular.module("insight.connection", []), angular.module("insight.currency", []), angular.module("insight.messages", []), angular.module("insight.address").controller("AddressController", function($scope, $rootScope, $routeParams, $location, Global, Address, getSocket) {
    ...
    $rootScope.currency.getConvertion = function(value) {
        if (value = 1 * value, !isNaN(value) && "undefined" != typeof value && null !== value) {
            ...
            var response;
            return "USD" === this.symbol ? response = _roundFloat(value * this.factor, 2) : "mXAI" === this.symbol ? (this.factor = 1e3, response = _roundFloat(value * this.factor, 5)) : "bits" === this.symbol ? (this.factor = 1e6, response = _roundFloat(value * this.factor, 2)) : (this.factor = 1, response = value), 1e-7 > response && (response = response.toFixed(8)), response + " " + this.symbol
        }
        return "value error"
        ...
        }) : $rootScope.currency.factor = "mXAI" === currency ? 1e3 : "bits" === currency ? 1e6 : 1
...
angular.module("insight.system").controller("IndexController", function($scope, Global, getSocket, Blocks) {
    ...
    $scope.message = {
        ...
        $scope.verification.status = "loading", $scope.verification.address = $scope.message.address, $http.post("/api/messages/verify", $scope.message).success(function(data) {
    ...
    var pageNum = 0,
        ...
        _aggregateItems = function(items) {
            ...
            for (var l = items.length, ret = [], tmp = {}, u = 0, i = 0; l > i; i++) {
                var notAddr = !1;
                if (items[i].scriptSig && !items[i].addr && (items[i].addr = "Unparsed input address [" + u++ +"]", items[i].notAddr = !0, notAddr = !0), items[i].scriptPubKey && !items[i].scriptPubKey.addresses && (items[i].scriptPubKey.addresses = ["Unparsed output address [" + u++ +"]"], items[i].notAddr = !0, notAddr = !0), items[i].scriptPubKey && items[i].scriptPubKey.addresses.length > 1) items[i].addr = items[i].scriptPubKey.addresses.join(","), ret.push(items[i]);
                else {
    ...
    $scope.transaction = "", $scope.status = "ready", $scope.txid = "", $scope.error = null, $scope.formValid = function() {
        ...
        $scope.status = "loading", $http.post("/api/tx/send", postData).success(function(data) {
            ...
            $scope.status = "error", $scope.error = data ? data : "No error message given (connection error?)"
        })
    }
...
angular.module("insight").directive("scroll", function($window) {
    ...
    gettextCatalog.setStrings("es", {
        ...
        "A.I. Coin node information": "Información del nodo A.I. Coin",
        Block: "Bloque",
        ...
        "Last Block Hash (aicoind)": "Último Bloque Hash (aicoind)",
        ...
        "Best Block": "最良ブロック",
        "A.I. Coin comes with a way of signing arbitrary messages.": "A.I. Coinには任意のメッセージを署名する昨日が備わっています。",
        "A.I. Coin node information": "A.I. Coinノード情報",
        Block: "ブロック",
        "Block Reward": "ブロック報酬",
        Blocks: "ブロック",
        ...
        Input: "入力",
        "Last Block": "直前のブロック",
        "Last Block Hash (aicoind)": "直前のブロックのハッシュ値 (aicoind)",
        ...
        "This form can be used to broadcast a raw transaction in hex format over\n        the A.I. Coin network.": "このフォームでは、16進数フォーマットの生のトランザクションをA.I. Coinネットワーク上に配信することができます。",
        "This form can be used to verify that a message comes from\n        a specific A.I. Coin address.": "このフォームでは、メッセージが特定のA.I. Coinアドレスから来たかどうかを検証することができます。",
        "Time Offset": "時間オフセット",
        Timestamp: "タイムスタンプ",
        Today: "今日",
        "Total Amount": "A.I. Coin総量",
        "Total Received": "総入金額",
        "Total Sent": "総送金額",
